...
export default function NotificationBell({ initialUnread }: Props) {
  ...
  // Poll for unread count every 30s (works without Redis/SSE)
  ...
  // Re-check on window focus (tab switch back)
  useEffect(() => {
    window.addEventListener('focus', fetchUnread)
    return () => window.removeEventListener('focus', fetchUnread)
  }, [fetchUnread])

  // Fetch + mark read when dropdown opens
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className="relative flex items-center gap-1.5 text-sm text-gray-600 hover:text-emerald-600 font-medium transition-colors focus:outline-none"
          ...
          <div className="max-h-[360px] overflow-y-auto">
            {loading ? (
              ...
              <ul className="divide-y divide-gray-100">
                {notifications.map((n) => (
                  <li key={n.id} className={`flex gap-3 px-4 py-3 ${!n.isRead ? 'bg-emerald-50/50' : ''}`}>
